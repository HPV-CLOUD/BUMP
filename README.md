...
![Screenshot_1](https://0x0.st/s/IJNc4l6NZPMo1DVvtIOC9w/X3iP.png)

# 🟣 BUMP AutoBot /// HPV /// V3.01 🟣

| **Возможности**                                                    | **Поддерживается**  |
|----------------------------------------------------------------|:---------------:|
| **Многофункциональный** — *автоматизирует действия, которые вы обычно выполняете вручную:*<br>**—** *Получение ежедневной награды*;<br>**—** *Выполнение заданий*;<br>**—** *Сбор монет и запуск фарма*;<br>**—** *Сбор монет за рефералов*<br>**—** *Покупка буста*<br>**—** *Ожидание от 10 до 13 часов*;<br>**—** *Повторение действий через 10-13 часов.<br>Все действия выполняются в случайном порядке каждый раз, когда скрипт начинает сбор. Также добавлена пустая функция просмотра NFT коллекций* |✅|
...
| **Многопоточный** — *благодаря использованию технологии потоков, скрипт позволяет одновременно взаимодействовать с несколькими аккаунтами. Ограничений по количеству аккаунтов нет* |✅|
| **Proxy** — *скрипт поддерживает добавление неограниченного количества прокси-серверов. Даже если количество прокси меньше, чем количество аккаунтов, они будут равномерно распределены между всеми аккаунтами после проверки на валидность. Хотя использование прокси не является обязательным, настоятельно рекомендуется их использование для предотвращения блокировки в BUMP (не в Telegram)* |✅|
| **Точечная настройка** — *скрипт позволяет задать желаемое количество монет, получаемых за один цикл сбора монет. В конфигурации можно указать диапазон чисел, в пределах которого начисляются поинты. Помимо этого, вы можете настроить автоматическую покупку буста X5 для повышения эффективности* |✅|
...
**<br>Данный скрипт предназначен исключительно для образовательных целей. Пользуйтесь им ответственно и избегайте злоупотреблений. Вы полностью несете ответственность за всевозможные последствия его использования, включая риск блокировки аккаунта в боте BUMP (не в Telegram)!**
***
</div>
...
- **Linux**
  ...
  - ```
    ...
    ```
  - ```
    git clone https://github.com/HPV-CLOUD/BUMP
    ```
  - ```
    pip3 install HPV_TEAM_BUMP
    ```
- **Windows**
...
- Вы можете настроить желаемое количество монет, получаемых за один цикл сбора монет, указав диапазон чисел в конфигурационном файле. Для изменения конфигурации перейдите в ***Core > Config*** и откройте ***HPV_Config.py***. Найдите переменную ***COINS*** ***([СКРИНШОТ](https://telegra.ph/file/789a9bd5423b2f57e0a1c.jpg))***, в которой хранится диапазон значений. По умолчанию установлено значение от ***37,951 до 81,159***, что является оптимальным. Если хотите, вы можете изменить этот диапазон, но увеличивать максимальное значение выше 200,000 не рекомендуется. В лучшем случае монеты не будут засчитаны на баланс, в худшем — ваш аккаунт может быть заблокирован в BUMP (не в Telegram).
- Помимо этого, можно настроить автоматическую покупку буста X5 для повышения эффективности. Для изменения конфигурации перейдите в ***Core > Config*** и откройте ***HPV_Config.py***. Найдите переменную ***BUY_BOOST***. По умолчанию значение установлено на ***True***, что включает автоматическую покупку буста. Вы можете изменить значение на ***False***, чтобы отключить эту функцию.
...
- **Инструкция по получению уникальной ссылки:**
  ***
  **1)** *Перейдите в [бота](https://t.me/MMproBump_bot?start=ref_1295320967);*<br>
  **2)** *Введите команду **/start**;*<br>
  **3)** *Нажмите на кнопку **"Start Game"***;<br>
